...
class SharedPreferencesService extends LocaKeyValueStorage {
  ...
  Future<Result<String?>> fetch(String key) async {
    try {
      final sharedPreferences = await SharedPreferences.getInstance();
      _log.finer('Got token from SharedPreferences');
      return Result.ok(sharedPreferences.getString(key));
    } on Exception catch (e) {
      _log.warning('Failed to get token', e);
      return Result.error(e);
    }
  ...
  @override
  Future<Result<void>> save(String key, String? value) async {
    try {
      final sharedPreferences = await SharedPreferences.getInstance();
      if (value == null) {
        _log.finer('Removed token');
